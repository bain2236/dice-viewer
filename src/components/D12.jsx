...
const D12 = function ({ material, environment, animation }) {
  const { nodes } = useGLTF('/D12.glb');
  console.log(nodes);
  const geo = useMemo(() => toConvexProps(nodes.D12.geometry), [nodes]);
  const [ref] = useConvexPolyhedron(() => ({
    mass: 50, args: geo, rotation: [3, 3, 3],
  }));
  const { geometry } = nodes.D12;
  const symbols = nodes.D12symbols.geometry;
  const walls = nodes.D12walls.geometry;
