...
const D6 = function ({ material, environment, animation }) {
  const { nodes } = useGLTF('/D6.glb');
  console.log(nodes);
  const geo = useMemo(() => toConvexProps(nodes.D6.geometry), [nodes]);
  const [ref] = useConvexPolyhedron(() => ({
    mass: 50, args: geo, rotation: [3, 3, 3],
  }));
  const { geometry } = nodes.D6;
  const symbols = nodes.D6symbols.geometry;
  const walls = nodes.D6walls.geometry;
