...
const D20 = function ({ material, environment, animation }) {
  const { nodes } = useGLTF('/D20.glb');
  console.log(nodes);
  const geo = useMemo(() => toConvexProps(nodes.D20.geometry), [nodes]);
  const [ref] = useConvexPolyhedron(() => ({
    mass: 50, args: geo, rotation: [3, 3, 3],
  }));
  const { geometry } = nodes.D20;
  const symbols = nodes.D20symbols.geometry;
  const walls = nodes.D20walls.geometry;
