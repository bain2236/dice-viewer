...
const D8 = function ({ material, environment, animation }) {
  const { nodes } = useGLTF('/D8.glb');
  console.log(nodes);
  const geo = useMemo(() => toConvexProps(nodes.D8.geometry), [nodes]);
  const [ref] = useConvexPolyhedron(() => ({
    mass: 50, args: geo, rotation: [3, 3, 3],
  }));
  const { geometry } = nodes.D8;
  const symbols = nodes.D8symbols.geometry;
  const walls = nodes.D8walls.geometry;
