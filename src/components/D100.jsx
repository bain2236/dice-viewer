...
const D100 = function ({ material, environment, animation }) {
  const { nodes } = useGLTF('/D100.glb');
  console.log(nodes);
  const geo = useMemo(() => toConvexProps(nodes.D100.geometry), [nodes]);
  const [ref] = useConvexPolyhedron(() => ({
    mass: 50, args: geo, rotation: [3, 3, 3],
  }));
  const { geometry } = nodes.D100;
  const symbols = nodes.D100symbols.geometry;
  const walls = nodes.D100walls.geometry;
